...
{
    public function apiTryon(Request $request)
    {
        $errors = [
            0 => '成功',
            1 => '未提供api_key',
        ...
        if ($allow_host !== 'http://localhost:8000') {
//    if ($allow_host !== $request->header()->get('Origin')) {
    ...
    {
        $face_id = $request->get('face') ?: 1;
        $glass_id = $request->get('glass') ?: 0;

        return view('facepp.facepp')->with('info',[
            'glass' => CustomizationSubtype::where('customizable_type','frame')->get()[$glass_id],
            'face' => FaceImage::find($face_id)
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $upload_image = $request->file('face_image');
